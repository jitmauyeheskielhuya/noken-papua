...
<div class="page-wrapper">
  <div class="content container-fluid">
    <h4 class="text-emerald-400 pb-3">Halaman Edit Subkriteria Produk</h4>
    <div class="">
      <div class="row">
        <div class="col-6 mx-60 ml-10 bg-slate-200 p-3 rounded-md pr-5">
          <form action="<?= base_url('produk/update_kriteria_produk'); ?>" method="post">
            ...
            <div class="mb-4">
              ...
              <select name="id_subkriteria" class="w-full border-gray-300 focus:border-indigo-500 rounded-md shadow-md py-2 px-2" id="" name="id_subkriteria">
                <?php foreach ($subkriteria as $value) : ?>
                  <option value="<?= $value['id_subkriteria']; ?>" <?= ($value['id_subkriteria'] == $id_subkriteria) ? 'selected' : '' ?>><?= $value['nama_subkriteria']; ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            ...
            <div class="mb-4">
              <input type="hidden" name="id_kriteria_produk" class="w-full border-gray-300 focus:border-indigo-500 rounded-md shadow-md py-2 px-2" value="<?= $id_kriteria_produk ?>">
            </div>
